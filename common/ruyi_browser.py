"""ruyi 浏览器公共能力:启动栈 + 反检测注入 + 资源拦截。

从 register_outlook_ruoyi 抽出,register/unlock 共用,避免两边各改一遍。
本模块自包含(不 import register),依赖的小工具(log/_try_option_call/_env_bool/
_all_contexts/_mask_ua)就地复制。

两层入口:
- build_browser_options(tb,...):启动前配 FirefoxOptions(UA + quiet_prefs +
  无头 options + 可选 set_argument(url) 直接启动)。
- after_launch(page,...):启动后注入反检测(hardwareConcurrency=16 等)+ 资源拦截。

反检测注入(hardwareConcurrency=16)是 PX 按压放行的关键:本机 20 线程会触发
px-cloud.net/v 二次审查导致按压不放行,改 16 对齐正常机器(实测 PX 秒过)。
有头/无头都必须注入(register 3 处调用全无条件),故 after_launch 无条件调用。
"""
...
def log(msg, level="INFO"):
    """精简 log,格式与 register.log 一致([HH:MM:SS] [LEVEL] msg),不做级别过滤。

    register 侧有自己的完整 log(含 PROD/DEBUG 过滤),公共模块这里只用基础打印,
    保证日志风格一致即可。"""
...
def build_browser_options(tb, *, tag, user_agent, is_headless, direct_url=None):
    ...
    对应 register 9697-9741 的 tb 配置序列。direct_url=None 时不 set_argument(回退 page.get)。
...
def _apply_ruoyi_headless_page_patches(page, tag=None, log_once=False, user_agent=None, wait_loaded=True):
    ...
    有头/无头都调(register 3 处调用全无条件)。PX 放行依赖 hardwareConcurrency=16。
    wait_loaded:True(默认,register 用)用 run_js_loaded 等 doc_loaded 再注入;
...
def after_launch(page, *, tag, user_agent, block_resources=False, wait_loaded=True):
    ...
    对应 register 9895-9901。反检测注入无条件执行(有头无头都需要)。
    wait_loaded:True(默认,register)patch 等 doc_loaded;False(unlock 首屏)立即注入,
